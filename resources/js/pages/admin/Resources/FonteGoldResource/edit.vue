...
const form = useForm({
  weight_grams_min: props.fontegold.weight_grams_min,
  weight_grams_max: props.fontegold.weight_grams_max,
  densite: props.fontegold.densite,
  bareme_designation_carat_id: props.fontegold.bareme_designation_carat_id,
  purity_estimated: props.fontegold.purity_estimated,
  price_per_gram_local: props.fontegold.price_per_gram_local,
  total_price: props.fontegold.total_price,
  fonte_date: props.fontegold.fonte_date.split('T')[0],
  local_rate_id: props.fontegold.local_rate_id
})
...
function submitForm() {
  form.put(props.resource.routes.update.replace(':id', props.fontegold.id), {
    onSuccess: () => toast.success('Fonte mise à jour avec succès'),
...
<template>
  <Head :title="`Modifiez Fonte d'Or`" />

<AppLayout>
  <div class="flex flex-col gap-6 p-4 max-w-4xl mx-auto">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-2xl font-bold text-gold-600">Modifier Fonte d'Or</h1>
        <p class="text-sm text-muted-foreground">Modifiez fonte</p>
      </div>
