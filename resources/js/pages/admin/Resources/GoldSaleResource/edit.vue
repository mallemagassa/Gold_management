...
const form = useForm({
  shipment_id: props.goldsale.shipment_id,
  buyer_name: props.goldsale.buyer_name,
  weight_sold: props.goldsale.weight_sold,
  price_per_gram: props.goldsale.price_per_gram,
  total_price: props.goldsale.total_price,
  currency: props.goldsale.currency || 'XOF',
  sale_date: props.goldsale.sale_date.split('T')[0],
  payment_status: props.goldsale.payment_status,
  invoice_number: props.goldsale.invoice_number,
  created_by: props.goldsale.created_by
})
...
function submitForm() {
  form.put(props.resource.routes.update.replace(':id', props.goldsale.id), {
    onSuccess: () => {
...
<template>
  <Head :title="`Modifier Vente #${goldsale.invoice_number}`" />

  <AppLayout>
    <div class="flex flex-col gap-6 p-4 max-w-4xl mx-auto">
      <div class="flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-bold text-gold-600">Modifier Vente d'Or</h1>
          <p class="text-sm text-muted-foreground">Facture #{{ goldsale.invoice_number }}</p>
        </div>
